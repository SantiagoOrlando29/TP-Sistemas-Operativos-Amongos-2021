...
void superbloque_generar_estructura()
{
	...
	printf("superbloque.blocks %d\n", superbloque.blocks);
	printf("superbloque.block_size %d\n", superbloque.block_size);
...
//TODO PUEDE QUE HAYA ALGUNA FALLA CON LA GENERACION BITMAP DEL SUPERBLOQUEPORQUE DESPUES NO SE PUEDE OBTENER BIEN EL BLOQUE LIBRE
//(EN ALGUN MOMENTO SE HABIA PODIDO)
...
void metadata_generar_cantidad(t_recurso_md* recurso_md, int cantidad)
{
	...
	while(cantidad > 0)
	{
		...
		if(cantidad > 0)
		{
			...
			{
				//FIXME log_error+return o no dar bola a la condicion de que no hayan bloques libres ya que sino habria que se hacer un rollback trabajando todo en un buffer
...
{
	log_debug(logger, "I-Entro a cadena_eliminar_array_de_cadenas");
	...
	*puntero_a_array_de_punteros = NULL;
	log_debug(logger, "O-Salgo de cadena_eliminar_array_de_cadenas");
}
...
//FIXME Busca y devuelve el primer bloque libre en el bitmap, si no existiera finaliza con error
...
//	superbloque.bitmap = 0;//asignacion para debugguear pero que no deberia estar
	t_bitarray* bitmap = bitarray_create_with_mode(superbloque.bitmap, superbloque_size, LSB_FIRST);
	int bloque;
	log_debug(logger, "Aca rompo");printf("y no se porque no muestro esto ");

	bitarray_test_bit(bitmap, 1);

	printf("%d ", bitarray_test_bit(bitmap, 2));
	printf("%d ", bitarray_test_bit(bitmap, 3));
	for(bloque = 0; bloque < superbloque.blocks && bitarray_test_bit(bitmap, bloque); bloque++)
		printf("%d ", bitarray_test_bit(bitmap, bloque));
	log_debug(logger, "\nSe ingresa a superbloque_obtener_bloque_libre4");
	...
	log_debug(logger, "y esto menos");
	bitarray_destroy(bitmap);

	log_debug(logger, "O-Bloque libre obtenido %d", bloque);
	return bloque;
}
...
{
	log_debug(logger, "I-Cadena original %s", cadena);

	int tamanio_final_cadena = strlen(cadena);
	cadena = realloc(cadena, tamanio_final_cadena);
	cadena[tamanio_final_cadena - 1] = '\0';

	log_debug(logger, "O-Cadena despues de sacarle el ultimo caracter %s", cadena);
}
...
void superbloque_actualizar_archivo()
{
	...
	log_debug(logger, "O-Archivo SuperBloque.ims actualizado con los valores de la memoria");
}
